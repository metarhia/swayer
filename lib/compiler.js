...
const resolveFactories = async (factoryProvider) => {
  if (!factoryProvider) return {};
  const factories = {};
  for (const [name, factoryModule] of Object.entries(factoryProvider)) {
    factories[name] = await factoryModule;
  }
  return factories;
};
...
const compileComponent = async (schema, container) => {
  const inputs = {
    elements: await resolveFactories(schema.elements),
    domain: await resolveFactories(schema.domain)
  };
