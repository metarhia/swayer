import Binding, {
  ElementBinding,
  ExistingBinding, ExistingElementBinding,
  TextBinding,
} from './binding.js';
...
export default class HydrationCompiler extends SchemaCompiler {
  ...
  async createComponent(input, existing) {
    const schema = await this.createSchema(input);
    schema.createBinding(() => this.createExistingBinding(schema, existing));
    this.setSchemaRenderer(schema);
    schema.init(this);
  }

  createExistingBinding(schema, existing) {
    return new ExistingElementBinding(schema, existing, this.webApi);
  }
