...
export default class Reflector {
  ...
  static reflectAll(context) {
    ...
    /* eslint-enable */
    for (const Type of reflections) this.reflect(context, Type);
  }

  static reflect(context, ReflectionType) {
    const reflection = new ReflectionType(context);
...
class AttrsReflection {
  ...
  #reflectInlineStyle(attrs = this.target[this.property]) {
    if (!attrs.style) return;
    Reflector.reflect(this.#context, InlineStyleReflection);
  }
}
...
class ComputedStylesReflection {
  ...
  setTargetPropertyProp(prop, value) {
    ...
    this.#context.styler.changeStyles(this.#context, prevStyles, value);
  }
}
...
class StylesReflection {
  ...
  #reflectComputedStyles(styles = this.target[this.property]) {
    if (!styles.compute) return;
    Reflector.reflect(this.#context, ComputedStylesReflection);
  }
}
