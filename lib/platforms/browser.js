...
class JITCompiler extends SchemaCompiler {
  async run(rootInput, mountElement) {
    const generator = await this.generateComponents(rootInput);
    const { value: schema } = await generator.next();
    if (mountElement) schema.renderer.mount(mountElement);
