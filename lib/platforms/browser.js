import { DocumentBinding, ElementBinding, TextBinding } from '../binding.js';
import {
  APP_ATTR,
  CLASS_PREFIX,
  HASH_ATTR,
...
import { HeadRenderer, Renderer } from '../renderer.js';
...
class TextHydrationRenderer extends Renderer {
  createBinding() {
    const schema = this.context.schema;
    const webApi = this.webApi;
...
class BrowserPlatform {
  ...
  #createEngine(mode) {
    ...
    this.#engine.setRendererTypes({
      ...
      html: createElementHydrationRenderer(Renderer),
      head: createElementHydrationRenderer(HeadRenderer),
      element: createElementHydrationRenderer(Renderer),
    });
  }
