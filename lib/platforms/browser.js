...
// todo use const schema = { path: mountElement.dataset[SRC_ATTR] };
// for partial application start
...
class BrowserPlatform {
  ...
  start() {
    const bootstrap = async () => {
      ...
      const { schema, router } = await this.#createApplication();
      if (!schema) return;
      const sheet = this.#getStyleSheet(mountElement);
      const compilation = await this.#startCompilation(schema, router, sheet);
  ...
  #loadConfig() {
    ...
    return this.#loader.loadEntry(appUrl);
  }

  async #startCompilation(schema, router, styleSheet) {
    const rootContext = await this.#compiler.createRoot(schema);
    rootContext.router = router;
  ...
  async #compileCSRPage(compilation) {
    const generator = this.#compiler.proceed(compilation);
  ...
  async #createApplication(urlPath = this.#location.pathname) {
    const { entry, entryUrl } = this.#loader;
    const routes = entry?.routes;
    if (!routes) throw Reporter.error('RootRoutesNotFound', { entryUrl });
    const reload = (path) => this.#reloadApplication(path);
    const rootContext = {
      schema: { tag: 'html' },
      module: { url: entryUrl },
    };
    const router = new Router(rootContext, routes, reload, urlPath);
    const schema = await router.matchSchema();
    return { router, schema };
  }

  async #reloadApplication(urlPath) {
    const { schema, router } = await this.#createApplication(urlPath);
    if (schema) await this.#renderFullPage(schema, router);
  }

  async #renderFullPage(schema, router) {
    const root = await this.#compiler.createRoot(schema);
    root.router = router;
    const style = this.#dom.createElement('style');
    const textRuleHandler = (rule) => (style.textContent += rule);
    root.createStyler(textRuleHandler);
    ...
    root.createStyler(ruleHandler);
    for await (const context of generator) {
      if (typeof context.schema === 'string') continue;
      if (context.schema.tag === 'body') {
        this.#dom.documentElement.replaceWith(root.binding.getNativeNode());
        await drainGenerator(generator);
      }
    }
  }
