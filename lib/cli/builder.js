...
export default class Builder {
  ...
  async build(options) {
    ...
    await this.#copyEnv(env, outputDir);
  ...
  async #copyEnv(env = 'development', destDir) {
    try {
      await fsp.access(ENV_DIR);
    } catch {
      return;
    }
    const dirents = await fsp.readdir(ENV_DIR);
    const envFile = dirents.find((name) => {
      const middleExt = extname(basename(name, '.js')).slice(1);
      return middleExt === env;
    });
    if (!envFile) throw Reporter.error('EnvNotFound', { env, dir: ENV_DIR });
    const srcPath = resolve(ENV_DIR, envFile);
    const destPath = resolve(destDir, ENV_FILE_NAME);
