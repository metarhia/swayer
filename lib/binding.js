...
export default class NodeBinding {
  ...
  constructor(webApi, schema, existingElement) {
    this.#webApi = webApi;
    this.#schema = schema;
    const dom = webApi.document;
    if (existingElement) {
      this.#node = existingElement;
      this.hydrated = true;
    } else if (is.str(schema)) {
      this.#node = dom.createTextNode(schema);
    } else {
      this.#node = dom.createElement(schema.tag);
