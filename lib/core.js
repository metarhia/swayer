...
export class Context extends BaseContext {
  ...
  destroy() {
    this.callLifecycleHook('destroy');
    this.cleanUp();
    super.destroy();
  }
  ...
  cleanUp() {
    ...
    for (const segment of this.children) {
      for (const context of segment) {
        context.callLifecycleHook('destroy');
        context.cleanUp();
      }
    }
  }

  async callLifecycleHookAsync(hook) {
    try {
      await this.#callHook(hook);
    } catch (error) {
  ...
  #callHook(hook) {
    ...
    const hookHandler = component.hooks[hook];
    if (hookHandler) return hookHandler.call(component);
  }

  #handleHookError(hook, error) {
    const schema = this.schema;
    const moduleUrl = this.module.url;
    const data = { hook, error, schema, moduleUrl };
    Reporter.errorLog('Lifecycle', data);
  }
}
...
export default class SwayerEngine {
  ...
  async *start(startContext) {
    ...
    const collectFinalizers = (context) => {
      ...
      if (component?.hooks.ready) readiness.push(context);
    ...
    try {
      ...
      for (const context of readiness) {
        await runMacroTask();
        await context.callLifecycleHookAsync('ready');
      }
