...
class ContextFactory {
  ...
  static #validateInput(input) {
    ...
    const isBadState = input?.state && !isObject(input.state);
    if (isBadState) throw error('state');
    const isBadModel = input?.state?.model && !isObject(input.state.model);
    if (isBadModel) throw error('model');
  ...
  async createContext(inputSchema, parent, recurrent = []) {
    ...
    const originalSchema = ContextFactory.#validateInput(inputSchema);
    let schema = originalSchema;
    let module = parent?.module;
    if (this.#baseTypes.includes(typeof schema)) {
