...
class ContextFactory {
  ...
  async *generateContexts(input, parent) {
    if (!input) return;
    const rootContext = await this.createContext(input, parent);
    const stack = Array.isArray(rootContext) ? rootContext : [rootContext];
  ...
  async #initSegments(context, stack) {
    if (!Array.isArray(context.schema.children)) return;
