...
class BaseContext {
  ...
  createStyler(ruleHandler) {
    this.styler = new Styler(ruleHandler);
  }
...
export default class SchemaCompiler {
  ...
  async start(rootContext) {
    ...
    const { binding, component } = root;
    root.styler.setStyles(binding, component?.styles);
  ...
  async *proceed(compilation) {
    ...
    const registerInitHook = (component) => {
      ...
      if (init) safeInits.push(init.bind(component));
    };
