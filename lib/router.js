...
export default class Router {
  ...
  constructor(context, routes, reload, initialPath = '') {
    this.#history = context.engine.webApi.history;
    this.#routes = routes;
  ...
  registerHistoryChanges() {
    const webApi = this.context.engine.webApi;
    this.#popstateHandler = () => {
      const path = webApi.location.pathname;
      this.context.setInitialPath(path);
      this.reload();
    };
    webApi.addEventListener('popstate', this.#popstateHandler);
  }

  unregisterHistoryChanges() {
    const webApi = this.context.engine.webApi;
    if (this.#popstateHandler) {
      webApi.removeEventListener('popstate', this.#popstateHandler);
    }
  }
  ...
  go(path) {
    if (path.startsWith(PATH_DELIMITER)) {
      ...
      router.context.setInitialPath(path);
      router.reload();
    } else {
      const prefix = this.parent.originalPath;
      const delimiter = prefix.endsWith(PATH_DELIMITER) ? '' : PATH_DELIMITER;
      const newPath = prefix + delimiter + path;
      this.#pushState(newPath);
      const children = this.parent.children;
      const reloadChild = async (child) => {
        child.path = path;
        const match = await child.matchSchema();
        child.reload(match);
      };
      void Promise.all(children.map(reloadChild));
    }
  }
  ...
  #pushState(path, state = {}) {
    if (isBrowser) this.#history.pushState(state, '', path);
  }
