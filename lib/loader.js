...
export default class Loader {
  static ENTRY_FILE_NAME = 'main.js';
  entry;
  #moduleCache = {};
  #namespaces;
  #origin;
  ...
  get entryUrl() {
    const delimiter = this.#origin.endsWith('/') ? '' : '/';
    return this.#origin + delimiter + Loader.ENTRY_FILE_NAME;
  }

  async loadEntry(appUrl) {
    this.#origin = appUrl;
    try {
      const entryUrl = this.entryUrl;
      this.entry = (await import(entryUrl)).default;
      this.#namespaces = this.entry.namespaces;
    } catch {
      return undefined;
    }
    return this.entry;
