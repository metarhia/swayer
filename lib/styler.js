...
import { camelToKebabCase, isObject } from './utils.js';
...
export default class Styler {
  ...
  changeStyles(context, prevStyles, curStyles, isComputed) {
    if (!Styler.#hasAnyStyles(curStyles)) return;
    const { binding, schema } = context;
    const tag = isComputed ? schema.tag : '';
    const className = this.#makeClass(curStyles, tag);
  ...
  setStyles(context, styles, isComputed) {
    if (!Styler.#hasAnyStyles(styles)) return;
    const { binding, schema } = context;
    const tag = isComputed ? schema.tag : '';
    const className = this.#makeClass(styles, tag);
    binding.toggleClass(className, true);
  }
  ...
  static #hasAnyStyles(styles) {
    if (!isObject(styles)) return false;
